<?php
class TableFighters extends WP_List_Table
{
    ...
    function column_default( $item, $column_name ) 
    {
        ...
        { 
             case 'image':
                return '<img width="35px" height="35px" alt="" src="'.FANVICTOR_IMAGE_URL.Fighters::replaceSuffix($item['image']).'">';
            case 'name':
                return $item[ $column_name ];
            case 'edit':
                return sprintf('<a href="?page=%s&action=%s&id=%s">Edit</a>', 'add-fighters','edit',$item['fighterID']);
            default:
    ...
    function usort_reorder( $a, $b ) 
    {
        ...
        $orderby = ( ! empty( $_GET['orderby'] ) ) ? $_GET['orderby'] : 'fighterID';
    ...
    {
        return sprintf(
            '<input type="checkbox" name="id[]" value="%s" />', $item['fighterID']
        );    
    }
