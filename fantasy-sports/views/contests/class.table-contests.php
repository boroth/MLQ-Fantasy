...
class TableContests extends WP_List_Table
{
    ...
    function column_default( $item, $column_name ) 
    {
        ...
        { 
            ...
            case 'creator':
                return $item[ $column_name ];
            case 'opponent':
                echo strtolower($item[ $column_name ]);
                echo " | ";
                echo '<a onclick="return jQuery.admin.showInviteFriendDialog(\''.__('Invite friend', FV_DOMAIN).'\', '.$item['leagueID'].')" href="javascript:void(0)">'.__('Invite', FV_DOMAIN).'</a>';
                return;
            case 'status':
                return $item[ $column_name ];
            case 'action2':
                ...
                if(strtolower($item['status']) == "new" || ($item['is_mixing'] = 1 && $item['is_mixing_complete'] == 0))
                {
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'opponent' => __('Opponent', FV_DOMAIN),
